        ...
        teleports = teleports
            .map(t => {
                if (t.date) {
                    ...
                    t.displaydate = new Date(t.time * 1000).toLocaleDateString("en-US", options);
                }
                return t;
            })
...
export const updateTportTokenBalances12 = async function({rootGetters, dispatch}, injectedWeb3, web3, erc20abi ) {
  if (rootGetters["bridge/getToNative"]) {
    return dispatch('updateTportTokenBalancesEvm',{injectedWeb3, web3, erc20abi});
  } else {
...
export const updateTportTokenBalancesEvm = async function (
  { commit, getters, rootGetters }, injectedWeb3, web3, erc20abi
) {
  try {
    if (getters.getEvmChainId && getters.getEvmAccountName) {
      ...
      for (const token of tokens) {
        try {
          if (injectedWeb3) {
            console.log(wrongNetwork(getters.getEvmNetwork, rootGetters["bridge/getFromChain"]))
            if (wrongNetwork(getters.getEvmNetwork, rootGetters["bridge/getFromChain"])) balance = 0;
            else {
