export const updateSwapPool = async function({
    ...
    try {
        ...
        const swapPoolId = fromToken?.toTokens?.find(token => token.contract == toToken.contract && token.symbol == toToken.symbol)?.pool
        if (swapPoolId){
...
export const createMemo = async function({
    ...
    try {
        ...
        minReturn = parseInt(parseFloat(minReturn).toFixed(getters.getToToken.precision).toString().replace('.', ''))
